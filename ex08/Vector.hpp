...
#define DIVISION_BY_ZERO "Division by zero error"
...
template<typename T>
const double angle_cos(const Vector<T>& first, const Vector<T>& second)
{
    Vector first_copy(first);

    T dot_product = first.dot(second);
    double norms_sum = first.norm() * second.norm();
    if (norms_sum == 0)
        throw std::runtime_error(DIVISION_BY_ZERO);
